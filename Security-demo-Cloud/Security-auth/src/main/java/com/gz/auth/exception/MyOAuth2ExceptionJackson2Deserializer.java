...
public class MyOAuth2ExceptionJackson2Deserializer extends StdDeserializer<OAuth2Exception> {
  ...
  public OAuth2Exception deserialize(JsonParser jp, DeserializationContext ctxt) throws IOException,
    ...
    String errorMessage = errorParams.get(OAuth2Exception.DESCRIPTION) != null ? errorParams
        .get(OAuth2Exception.DESCRIPTION).toString() : null;
    ...
    if (OAuth2Exception.INVALID_CLIENT.equals(errorCode)) {
      ...
      if (errorMessage.toLowerCase().contains("redirect") && errorMessage.toLowerCase()
          .contains("match")) {
      ...
      ex = new InsufficientScopeException(errorMessage,
          OAuth2Utils.parseParameterList((String) errorParams
              .get("scope")));
